...
def arguments():
    ...
    parser.add_argument('-m', '--motevo', dest='motevobin', type = str, help = 'Path to binary of motevo program', required=True)  # 0 to 1
...
def clusterJob(path, wmfile, wmname, fastafile, outputDir, cutoffDir, param_file, param_dir, motevobin):
...
def runCommand(path, wmfile, wmname, fastafile, outputDir, cutoffDir, param_file, param_dir, motevobin):
    fname = os.path.join(path, wmname + '.sh')
    cmd = ' '.join([
        motevobin,
        fastafile,
...
def main():
    ...
    for wmfile, wmname in zip(WMFiles, WMNames):
        ...
        runCommand(path, wmfile, wmname, args.fastafile, args.outputDir, args.cutoffDir, "%s.param" % wmname, args.ParamDir, args.motevobin)
        # exit()
